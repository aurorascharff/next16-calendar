...
export function RadioCard({ children, className, ...props }: RadioCardProps) {
  return (
    <label className="relative block h-full cursor-pointer has-[:disabled]:cursor-not-allowed">
      ...
      <span
        className={cn(
          'border-divider text-muted hover:border-primary/45 peer-checked:border-primary peer-focus-visible:ring-primary/25 dark:border-divider-dark dark:hover:border-primary/60 peer-disabled:bg-card/35 peer-disabled:text-muted/50 peer-disabled:hover:border-divider dark:peer-disabled:bg-card-dark/35 dark:peer-disabled:hover:border-divider-dark flex h-full items-center rounded-md border bg-transparent px-4 text-left text-sm font-medium tabular-nums transition-[background-color,border-color,color,transform] duration-150 ease-out peer-checked:text-black peer-checked:shadow-[inset_0_0_0_1px_var(--color-primary)] peer-focus-visible:ring-2 peer-focus-visible:outline-none peer-disabled:cursor-not-allowed peer-disabled:line-through hover:text-black active:scale-[0.98] peer-disabled:active:scale-100 dark:peer-checked:text-white dark:hover:text-white',
          className,
        )}
      >
        {children}
