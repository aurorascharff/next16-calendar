...
export function CalendarMonthBoard({
  ...
  const visibleEvents = useOptimisticEvents(events, days).filter(event => !hidden.has(event.calendarId));
      ...
      <div className="col-start-1 row-start-2 grid auto-rows-[9rem] grid-cols-7 sm:auto-rows-[11rem]">
        {days.map(day => {
          ...
          return (
            <div
              className={cn(
                'group border-divider dark:border-divider-dark relative overflow-hidden border-r border-b p-1 transition-[background-color] duration-200 ease-out motion-reduce:transition-none sm:p-1.5',
